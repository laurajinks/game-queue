...
export default class Game extends Component {
    ...
    updateNotes = (e, id) => {
        axios
            .put(`${url}/api/games/${id}`, { notes: this.state.input })
            .then(response => {
                this.setState({
                    games: response.data,
                    input: "",
    ...
    render() {
        return (
            <div className="gameContainer">
                ...
                <button
                    onClick={() => this.deleteGame(this.props.id)}
                    className="deleteBtn"
                >
                    X
                </button>
                ...
                <div className="gameBody">
                    ...
                    />
                    <h2 className="gameTitle">{this.props.title}</h2>
                    <br />
                    <p className="notes">Notes: {this.props.notes}</p>

                    <div className="gameFooter">
                        {this.state.displayEditBtn === true && (
                            <button
                        ...
                        {this.props.queue === true && (
                            <button
                                onClick={() =>
                                    this.props.completeGame(this.props.id)
                                }
                                className="moveBtn"
                            >
                                Game Completed
                        ...
                        {this.props.queue === false && (
                            <button
                                onClick={() =>
                                    this.props.returnToQueue(this.props.id)
                                }
                                className="moveBtn"
                            >
                                Return to Queue
